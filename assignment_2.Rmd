...
For the knn model, data are first centred and scaled so 
as to not favour dimensions/variables based on their scale. The optimal `k` value
is searched for between 1 and 3 by specifying the `tuneGrid` argument.
```{r train-knn}
mod_knn <- train(classe ~ . ,data=training_refined_2,method='knn',tuneGrid=data.frame(k=1:3))
```
